...
import { UserBasicDTO, UserDTO } from "../models";
...
export const getParticipantAvatar = (user: EventParticipant) =>
  user?.avatarFile
    ? `${BASE_URL}images/${user.avatarFile}`
    : IMAGE_MOCK;

export const getFullNameParticipant = (user: EventParticipant) =>
  user ? `${capitalizeFirstLetter(user.name)} ${capitalizeFirstLetter(user.lastname)}` : "";